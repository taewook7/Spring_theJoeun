...
public class MemberController {
	...
	public String memberDelete(MemberDTO memberDTO, HttpSession session, 
		...
		// 세션에 있는 비밀번호를 추출한다. => 테이블에 저장된 정보
		String sessionPass = member.getPasswd();
		
		// memberDTO에 들어있는 비밀번호(memberDeleteView에서 입력한 비밀번호)
		String memberDTOPass = memberDTO.getPasswd();
		
		// 세션에는 회원에 대한 모든 정보(아이디, 비밀번호, 이름)이 들어있기 때문에
		// 테이블을 다시 검색하지 않아도 된다.
		// 세션에 담겨있는 비밀번호와 탈퇴하기 위해서 입력한 비밀번호가 
		// 맞을 경우만 탈퇴처리를 한다.
	...
	@ResponseBody
	@RequestMapping(value="/ResponseBodyCheck", method = RequestMethod.GET)
	public String ResponseBodyCheck(MemberDTO memberDTO) throws Exception {
	...
	// 아이디 중복 검사
	@ResponseBody // ajax로 요청한 곳으로 보내기 위해서 필요.
	@RequestMapping(value="/idCheck", method = RequestMethod.POST)
	public int idCheck(MemberDTO memberDTO) throws Exception {
		logger.info("MemberController idCheck.....");
		
		int result = memberService.idCheck(memberDTO);
		logger.info("MemberController idCheck Return Value [" + result + "]");
